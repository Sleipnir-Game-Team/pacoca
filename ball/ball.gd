...
func _ready() -> void:
	var base_velocity = Vector2(sin(deg_to_rad(direction)), cos(deg_to_rad(direction))) 
...
func change_angle(angle: float) -> void:
	direction = angle
	var base_velocity = Vector2(cos(deg_to_rad(direction)), sin(deg_to_rad(direction)))
	velocity = base_velocity * speed * heat.speed_bonus
